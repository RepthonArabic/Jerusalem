...
from ..helpers.utils import _reputils, reply_id, parse_pre, yaml_format, install_pip, get_user_from_event, _format
...
@zq_lo.rep_cmd(
    pattern="(اعاده تشغيل|اعادة تشغيل)$",
...
async def _(event):
    ...
    sandy = await edit_or_reply(
        event,
        f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗥𝗲𝗽𝘁𝗵𝗼𝗻 🝢 **إعــادة التشغيــل**\n"
        f"**•─────────────────•**\n\n"
        f"**•⎆┊اهـلا عـزيـزي** - {mention}\n"
        f"**•⎆┊يتـم الان اعـادة تشغيـل بـوت ريبـــثون **\n"
...
async def _(event):
    ...
    await edit_or_reply(event, "**✾╎جــارِ إيقـاف تشغيـل بـوت ريبـــثون الآن 📟 ...**\n\n**✾╎شغِّـلنـي يـدويًـا لاحقًــا**\n**✾╎باتبـاع الشـرح** https://t.me/zzzlvv/20")
...
async def _(event):
    ...
    o = stdout.decode()
    OUTPUT = (
        f"**[𝗦𝗢𝗨𝗥𝗖𝗘 𝗥𝗲𝗽𝘁𝗵𝗼𝗻](tg://need_update_for_some_feature/) - فـارات تنصيبـك هـي:**\n\n\n{o}"
    )
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗥𝗲𝗽𝘁𝗵𝗼𝗻](tg://need_update_for_some_feature/) - سرعـة السيرفـر**\n**- تم حسـاب سرعـة سيرفـر البـوت الخـاص بك :**\n\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await event.client.send_file(
...
async def reppp(event):
    uname = platform.uname()
    rept = "**- تاريخ تنصيبـك لـ بـوت ريبـــثون - 𓆩𝙎𝙊𝙐𝙍𝘾𝞝 𝙍𝙀𝙋𝙏𝙃𝙊𝙉𓆪**\n\n"
    boot_time_timestamp = psutil.boot_time()
    bt = datetime.fromtimestamp(boot_time_timestamp)
    rept += f"**- التاريـخ 📋:**\t`{bt.day}/{bt.month}/{bt.year}`\n**- الـوقت ⏰:**\t`{bt.hour}:{bt.minute}`\n"
    cpufreq = psutil.cpu_freq()
    for i, percentage in enumerate(psutil.cpu_percent(percpu=True)):
        svmem = psutil.virtual_memory()
